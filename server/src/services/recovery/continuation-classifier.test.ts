...
describe("classifyContinuationFailure", () => {
  ...
  describe("priority ordering (negative fixtures)", () => {
    ...
    it("does NOT classify the deliberate-wait code as fleet_gated", () => {
      const result = classifyContinuationFailure(makeRun("issue_continuation_waiting_on_review"));
      expect(result.kind).toBe("deliberate_wait_without_target");
    });
  });
});

// ── no-double-fire pin (ALM-7532 open question) ──────────────────────────────
// The fleet_gated gate is the ONLY recovery gate that reads the auth-outage
// codes. classifyAdapterFailureForRecovery early-returns null for any errorCode
// outside {adapter_failed, provider_quota, configuration_incomplete}, so an auth
// code can never also reach the provider-quota or configuration-incomplete
// lanes. These pin that non-interaction: if someone widens that allowlist to
// include an auth code, both gates would fire on one run and these go red.
describe("fleet_gated does not double-fire with the adapter-failure classifier", () => {
  it.each(["claude_auth_required", "acpx_auth_required"])(
    "classifyAdapterFailureForRecovery returns null for %s",
    (code) => {
      expect(
        classifyAdapterFailureForRecovery({ errorCode: code, error: null, resultJson: null }),
      ).toBeNull();
    },
  );

  it("returns null for an auth code even when the error text looks like a quota message", () => {
    expect(
      classifyAdapterFailureForRecovery({
        errorCode: "claude_auth_required",
        error: "You've hit your usage limit. Try again at 4:30 PM (America/Chicago).",
        resultJson: null,
      }),
    ).toBeNull();
  });

  it("returns null for an auth code even when the error text looks like missing credentials", () => {
    expect(
      classifyAdapterFailureForRecovery({
